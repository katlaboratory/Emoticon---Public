...
ggplot(dtRaw, aes(x = Feedback, y = measurement, fill = Valence, colour = Valence)) +
  ...
  geom_point(position = position_jitter(width = .30), size = .35, alpha = 0.8) +
  geom_point(position = position_dodge(width = 0.12), data = sC_summaryStats, 
             aes(x = as.numeric(Feedback), y = measurement_mean, group = Valence, colour = Valence),
             size = 3, shape = 18, alpha = 0.8) +
  geom_errorbar(position = position_dodge(width = 0.12), data = sC_summaryStats, 
                aes(x = as.numeric(Feedback), y = measurement_mean, group = Valence, 
                    ymin = measurement_mean - ci, ymax = measurement_mean + ci, colour = Valence), 
                width = 0.2, size = 1.5, alpha = 0.8) +  # Adjust the size parameter here
  geom_hline(yintercept = 0, linetype = "dashed") +
  ylab('Perceived Social Intent') +
  xlab('Feedback Mode') +
  ylim(c(0, 100)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        plot.caption = element_text(hjust = 0.5), text = element_text(size = 16))
ggsave('Hypothesis_1_revised.png', width = 8, height = 6, dpi = 1000)

# REVISION - GRAPH ON HR #### 
packages <- c("cowplot", "readr", "ggplot2", "dplyr", "lavaan", "smooth", "Hmisc")
if (length(setdiff(packages, rownames(installed.packages()))) > 0) {
  install.packages(setdiff(packages, rownames(installed.packages())))
}
...
sC_summaryStats <- summarySE(dtRaw, measurevar = "HR_diff",na.rm = TRUE,
                             groupvars=c("Feedback", "Valence"))

ggplot(dtRaw, aes(x = Feedback, y = HR_diff, fill = Valence, colour = Valence)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2, trim = FALSE, alpha = 0.7) +  
  geom_point(position = position_jitter(width = .15), size = .25) +
  geom_point(position = position_dodge(width = 0.12), data = sC_summaryStats, 
             aes(x = as.numeric(Feedback), y = HR_diff_mean, group = Valence, colour = Valence),
             size = 3, shape = 18) +
  geom_errorbar(position = position_dodge(width = 0.12), data = sC_summaryStats, 
                aes(x = as.numeric(Feedback), y = HR_diff_mean, group = Valence,
                    ymin = HR_diff_mean - ci, ymax = HR_diff_mean + ci),  
                width = 0.2, size = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ylab('Baseline Corrected Heart Rate') +
  xlab('Feedback Mode') +
  ylim(c(-30, 30)) +
  scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.caption = element_text(hjust = 0.5), text = element_text(size = 16)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2, trim = FALSE, alpha = 0.7) +
  ggsave('Hypothesis_4_HR.png', width = 8, height = 6, dpi = 1000)

# REVISION - SCR GRAPH #####
sC_summaryStats <- summarySE(dtRaw, measurevar = "EDA_diff",na.rm = TRUE,
                             groupvars=c("Feedback", "Valence"))

ggplot(dtRaw, aes(x = Feedback, y = EDA_diff, fill = Valence, colour = Valence)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2, trim = FALSE, alpha = 0.5) +  
  geom_point(position = position_jitter(width = .15), size = .25) +
  geom_point(position = position_dodge(width = 0.12), data = sC_summaryStats, 
             aes(x = as.numeric(Feedback), y = EDA_diff_mean, group = Valence, colour = Valence),
             size = 3, shape = 18) +
  geom_errorbar(position = position_dodge(width = 0.12), data = sC_summaryStats, 
                aes(x = as.numeric(Feedback), y = EDA_diff_mean, group = Valence,
                    ymin = EDA_diff_mean - ci, ymax = EDA_diff_mean + ci),  
                width = 0.2, size = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ylab('Baseline Corrected Skin Conductance Rate') +
  xlab('Feedback Mode') +
  ylim(c(-3, 4)) +
  scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        plot.caption = element_text(hjust = 0.5), text = element_text(size = 16)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2, trim = FALSE, alpha = 0.3) +
  ggsave('Hypothesis_4_SCR.png', width = 8, height = 6, dpi = 1000)

# REVISION - HRV GRAPH ####
sC_summaryStats <- summarySE(dtRaw, measurevar = "RMSSD",na.rm = TRUE,
                             groupvars=c("Feedback", "Valence"))

ggplot(dtRaw, aes(x = Feedback, y = RMSSD, fill = Valence, colour = Valence)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2, trim = FALSE, alpha = 0.7) +  
  geom_point(position = position_jitter(width = .15), size = .25) +
  geom_point(position = position_dodge(width = 0.12), data = sC_summaryStats, 
             aes(x = as.numeric(Feedback), y = RMSSD_mean, group = Valence, colour = Valence),
             size = 3, shape = 18) +
  geom_errorbar(position = position_dodge(width = 0.12), data = sC_summaryStats, 
                aes(x = as.numeric(Feedback), y = RMSSD_mean, group = Valence,
                    ymin = RMSSD_mean - ci, ymax = RMSSD_mean + ci),  
                width = 0.2, size = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ylab('Heart Rate Variability') +
  xlab('Feedback Mode') +
  ylim(c(0, 0.2)) +
  scale_colour_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.caption = element_text(hjust = 0.5), text = element_text(size = 16)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2, trim = FALSE, alpha = 0.3) +
  ggsave('Hypothesis_4_HRV.png', width = 8, height = 6, dpi = 1000)
# REVISION - analyses on baseline pleasantness ###### 
dtRevision <- read_csv("Emoticon2_Revisions.csv", na=c("NA",""))

dtRevision_2<-dtRevision
dtRevision_2$OPTIMALITY<- relevel(as.factor(dtRevision_2$OPTIMALITY),ref="SUBOPTIMAL")

# scat opt sig lower than manual opt
lm1<-lmer(RATING~OPTIMALITY*MEDIUM + (1|subjId),data=dtRevision_2)
tab_model(lm1, show.se=2)
summary(lm1)
